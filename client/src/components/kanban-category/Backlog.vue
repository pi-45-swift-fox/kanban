<template>
  <div class="kanban">
    ...
    <div class="overflow-auto kanban-main">
      <div v-for="data in backlogProps" :key="data.id" class="card">
        <div class="card-body">
      ...
      <DeleteTaskModal @resetModal="moveTask" v-if="destroy"  v-if="destroy" @refreshPage="emit" :dataTask="data"></DeleteTaskModal>
      <MoveTaskModal @resetModal="moveTask" v-if="move"  v-if="move" @refreshPage="emit" :dataTask="data"></MoveTaskModal>

    </div>
    <div class="kanban-foot rounded-bottom">
      <h2 @click="toggleForm"  class="disable-select">add task.</h2>

      <form @submit.prevent="postBacklog" id="form-kanban-backlog" class='form-kanban' v-show="showForm">
...
<script>
  ...
  export default {
    ...
    methods: {
      toggleForm()
      {
        document.getElementById("form-kanban-backlog").reset();
